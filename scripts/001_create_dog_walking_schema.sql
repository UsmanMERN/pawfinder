...
create policy "dogs_select_own_or_walker_booking"
  on public.dogs for select
  using (
    auth.uid() = client_id or
    auth.uid() in (select walker_id from public.bookings where id in (select booking_id from public.bookings where dogs @> array[dogs.id::text]))
  );
...
create policy "walk_timeline_select"
  ...
  using (
    auth.uid() in (
      ...
      union
      select walker_id from public.bookings where id = booking_id
    )
  );
